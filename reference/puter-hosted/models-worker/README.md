...
1. Bu dosya, Puter Hosting üzerinde barındırılan models-worker kodunun birebir referans kopyasıdır; aktif deploy dosyası değildir.
2. Amaç, model kataloğunu açıklamak, belgelemek, Codex talimatlarında referans göstermek ve teknik anlamı tek yerde toplamaktır.
3. Dosyada modeller statik olarak tutulur; burada katalog verisi, filtre mantığı, sıralama kuralları ve API cevap yapısı bulunur.
4. Görsel, sohbet, reasoning, coding, arama, güvenlik ve ses odaklı modeller tek JSON benzeri veri kümesinde listelenir.
5. Yeni açıklamada Excel vurgusu kaldırılmalı; asıl vurgu, bunun referans amaçlı birebir kopya ve okunabilir katalog olmasıdır.

DETAYLAR

1. Bu referans dosyası, Puter Hosting üzerinde çalışan models-worker kodunun içerik bakımından birebir aynısıdır; üretimde doğrudan çalıştırılmaz.
2. Buradaki ana görev, kullanılabilir model kataloğunu tek noktada toplamak, açıklamak ve dış talimatlarda güvenilir referans sağlamaktır.
3. Dosya içinde modeller harici dosyadan okunmaz; tüm model kayıtları doğrudan kaynak kodun RAW_MODELS dizisinde bulunur.
4. Her model kaydında company, provider, modelName ve modelId alanlarıyla sağlayıcı, görünen ad ve teknik kimlik tutulur.
5. categoryRaw alanı modelin ana sınıfını belirtir; chat, multimodal, reasoning, coding, görsel, arama, güvenlik ve ses tipleri vardır.
6. badges alanı hızlı etiketleme sağlar; CHAT, MULTIMODAL, REASONING, CODING, GÖRSEL, ARAMA, AGENTIC, GÜVENLİK ve SES kullanılır.
7. parameters alanı, yaklaşık model boyutunu veya kullanılan mimari tipini insan okunur biçimde özetlemek için tutulur.
8. speedLabel alanı hız sınıfını taşır; ayrıca SPEED_SCORE_MAP ile bu yazılar otomatik sayısal speedScore değerine dönüştürülür.
9. Fiyat yapısı prices altında tutulur; input, output ve image değerleri ayrı ayrı gösterilir, uygun olmayanları null kalır.
10. traits alanı her model için en fazla beş kısa özellik taşır; hızlı tanım ve karşılaştırma için kullanılır.
11. standoutFeature alanı modelin ayırt edici gücünü tek cümlede açıklar; useCase ve rivalAdvantage alanları bağlamı tamamlar.
12. sourceUrl alanı katalog bilgisinin bağlı olduğu kaynak bağlantıyı saklar; böylece model kaydının menşei görünür olur.
13. style alanı üretim prompt stili değil, model kartının görsel kimliğini belirleyen brandKey ve accent bilgisini taşır.
14. Yani bu dosyada doğrudan anime, cinematic veya watercolor gibi stil filtreleri yoktur; stil daha çok arayüz temasıdır.
15. Buna rağmen bazı görsel modeller açıklama seviyesinde stil tutarlılığı, referans görsel anlama ve stil transfer yetenekleri belirtir.
16. Özellikle Flux Kontext Max ve Kontext Pro kayıtları, referans görsel uyumu ve tutarlı stil aktarımı vurgusunu taşır.
17. Recraft benzeri görsel modeller marka kimliği, SVG hassasiyeti ve kurumsal tasarım uyumu gibi yaratıcı avantajlar sunar.
18. Multimodal modeller metin yanında görsel, ses ve bazı kayıtlarda video anlama yeteneklerini açıklama katmanında gösterir.
19. Ayrı video-generation kategori anahtarı yoktur; video kabiliyeti bazı multimodal model açıklamalarında ve kullanım örneklerinde geçer.
20. Ses tarafı da sınırlı ama mevcuttur; Audio model kategorisinde PlayAI Dialog 1.0 ve SES etiketi bulunur.
21. API yüzeyinde desteklenen filtreler search, company, badge, category ve modelId alanlarıdır; bunlar sorguyla birlikte uygulanır.
22. Sıralama seçenekleri isim, şirket, input fiyatı, output fiyatı, image fiyatı ve hız skoruna göre artan-azalan çalışır.
23. limit ve offset parametreleri sayfalama içindir; varsayılan limit 50, üst sınır 250 olacak şekilde kısıtlanır.
24. GET /, GET /health ve GET /models rotaları; bilgi, sağlık kontrolü ve katalog listeleme cevaplarını üretir.
25. Dosya üstü açıklamada mutlaka bu kopyanın referans amaçlı olduğu, aktif deploy olmadığı ve Puter Hosting aynası olduğu yazılmalıdır.
