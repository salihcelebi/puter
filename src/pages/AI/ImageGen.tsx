...
async function requestImageGeneration(args: {
  ...
  console.debug('[ImageGen] generate request', {
    model: modelId,
    prompt,
    ratio,
    attachments: attachments.map((item) => item.name),
  ...
  if (!contentType.includes('application/json')) {
    if (response.status === 404) {
      throw new Error('Görsel servisi bulunamadı (404).');
    }
    throw new Error('Görsel servisi JSON yerine beklenmeyen yanıt döndürdü.');
  }

  let envelope: WorkerEnvelope<ImageResultPayload | null> | null = null;
  try {
    envelope = text ? (JSON.parse(text) as WorkerEnvelope<ImageResultPayload | null>) : null;
  } catch {
    throw new Error('Görsel servisi geçersiz JSON döndürdü.');
  }

  if (!envelope || typeof envelope.ok !== 'boolean' || !('code' in envelope) || !('data' in envelope)) {
    throw new Error('Görsel servisi standart zarf formatını döndürmedi.');
  }

  if (!response.ok || !envelope.ok) {
    throw new Error(envelope.error?.message || `Görsel üretimi başarısız (${envelope.code || response.status}).`);
  }
