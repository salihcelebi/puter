...
Bu README.md, ANO ve BABO worker’larının kaynak kodu açılmadan da yüksek doğrulukla yeniden kodlanabilmesi için oluşturuldu.
Bu belge, bir yapay zekânın veya geliştiricinin sistemin dış davranışını anlayıp uyumlu kod üretebilmesi için hazırlandı.
Bu README.md, endpoint’lerin görevini, girişlerini, çıkışlarını ve kurallarını tek yerde toplamak için yazıldı.
Bu belge, ANO ile BABO’nun rol farkını netleştirip yanlış mimari kurulmasını önlemek için oluşturuldu.
Bu README.md, kaynak kod görünmese bile route, response, yetki, KV ve iş akışlarının doğru kurulabilmesi için hazırlandı.
Bu belge, tahmine dayalı değil, açık teknik sözleşmeye dayalı kodlama yapılabilmesi için oluşturuldu.
Bu README.md, başka bir AI aracının sistemi eksik anlamadan, kontrollü ve tutarlı biçimde geliştirme yapabilmesi için yazıldı.
Bu belge, bakım, taşıma, yeniden yazım ve genişletme süreçlerinde resmi teknik referans olarak kullanılmak için hazırlandı.
Bu README.md, canlı davranışı koruyarak iç kod farklı olsa bile dış uyumun korunmasını sağlamak için oluşturuldu.
Bu belge, ANO ve BABO worker’larının kaynak kod görülmeden bile yüksek doğrulukla kodlanabileceğini net biçimde tanımlayan ana teknik sözleşmedir.
Bu belge, worker kimliği, response kimliği, meta dosya adı ve env sözlüğünü tek standarda bağlamak için güçlendirildi.
Bu belge, isim dönüşümü sırasında yarım migration riskini azaltmak için güvenli geçiş planını görünür kılar.
---
AMAÇ
...
Route adlarını değiştirmeyecek.
Request alanlarını bozmayacak.
Response sözleşmesini bozmayacak.
ANO ile BABO cevap şemalarını birbirine karıştırmayacak.
ANO’daki admin koruma mantığını gevşetmeyecek.
ANO’daki rate limit ve cache anahtarlarını koruyacak.
BABO’daki iş kaydı / geçmiş / arşiv omurgasını koruyacak.
BABO’daki timeout / retry / fallback / maliyet mantığını koruyacak.
PDF ve DEEPSEARCH’i tek adımlı yalancı akışa çevirmeyecek.
İç implementasyon birebir aynı olmak zorunda değildir; dış davranış birebir uyumlu olmalıdır.
Nihai sözleşmede sadece ANO ve BABO kimliklerini kullanacaktır.
Önceki worker kimliklerinden hiçbiri nihai response, meta, log, yorum ve README içinde bırakılmayacaktır.
---
...
ortak durum yazma
önbellek silme
sağlık, test, amaç ve tanıtım endpointleri
...
istemci kimliği
rate limit
SSE veya fallback JSON
`me.puter.ai.chat(..., { stream: true })`
text parçaları `parca` event’i
tool parçaları `arac` event’i
tamamlanan metni cache altında kaydet
`POST /api/gorsel`
Amaç:
txt2img üretimi
AI çağrısı:
...
girdi doğrula
etkin ayarı üret
hizmet yetkinliğini kontrol et
iş kaydı başlat
geçmişe `is_basladi` yaz
durumu `hazirlaniyor` yap
orkestra planı üret
durumu `isleniyor` yap
ana işlemi başlat
timeout uygula
hata olursa sınıflandır + yeniden deneme kararı ver
geçmişe hata yaz
gerekirse `yeniden_denemede`
son başarıda `tamamlandi`
geçmişe kalite kaydı yaz
sonuçları birleştir
arşive yaz
nihai cevap döndür
...
`worker: "BABO"`
AIAI sınıfları
`SINIF-1` → API ÇAĞIRAN İŞÇİLER
`SINIF-3` → ORKESTRA ŞEFİ
`SINIF-4` → İŞ TAKİP UZMANI
`SINIF-5` → TEST DEDEKTİFİ
...
Kodda önceki worker kimliklerini tam tara.
Kullanım türüne göre sınıflandır:
worker kimliği
dosya/meta kimliği
test anahtarı
KV anahtarı
...
response alanı
comment/metin
README’yi yeni sözlüğe göre güncelle.
Env sözlüğünü yeni isimlerle oluştur.
Önce iç raporlanan kimlikleri düzelt.
Sonra response/meta/log alanlarını düzelt.
Sonra test/sağlık endpointlerini düzelt.
Sonra KV/test prefix’lerini tutarlı hale getir.
Sonra tekrar tarama yap.
Nihai çıktıda yalnız ANO ve BABO kaldığını doğrula.
...
12.1 ANO testleri
`GET /api/durum` → `200`
`GET /api/modeller` → filtreleme ve `sinir`
`POST /api/sohbet` geçerli body → `200`
`POST /api/sohbet` bozuk JSON → `400`
`POST /api/sohbet` eksik model → `400`
`POST /api/sohbet/akis` → `hazir/parca/bitti`
SSE yoksa → `akisaUygunOrtam: false`
`POST /api/gorsel` eksik prompt → `400`
`GET /api/ayarlar/getir` → gizli yönetici alanı görünmez
ilk kurulum kısa admin anahtarı → `400`
yanlış admin ile ayar kaydetme → `403`
`POST /api/ortak-durum/yaz` yanlış admin → `403`
`POST /api/onbellek/sil` yanlış prefix → `400`
`GET /api/test/saglik` → KV metot alanları görünür
`GET /api/test/me-puter` → gerçek KV set/get/del
tüm sağlık ve test çıktıları `ANO` kimliğini döndürür
12.2 BABO testleri
`POST /api/calistir` CHAT → `200`
`POST /api/calistir` JSON bozuk → `400`
`POST /api/calistir` form-data parse → çalışmalı
serviceType yok ama prompt var → doğru tip çözülmeli
riskli URL → `DOGRULAMA_HATASI`
yasak mime → hata
timeout simülasyonu → `zaman_asimi_hatasi`
quota simülasyonu → retry/fallback kararı
`GET /api/is/:id` → iş özeti
`GET /api/is/:id/gecmis` → olay listesi
`GET /api/is/:id/arsiv` → arşiv
`GET /api/is/:id/izle` → queued → isleniyor dönüşümü
`POST /api/teshis` → çoklu rapor
`GET /api/saglayici/:hizmetTuru/:saglayici` başarısız erişim → `503`
`GET /api/ispat/ozet` → sağlık + panel
meta ve panel cevapları yalnız `BABO` kimliğini döndürür
---
